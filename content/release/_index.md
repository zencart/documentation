---
title: Release Documentation
description: Release Notes, lists of changed files, and other documents 
category: Zen Cart
no_list: true
---

## Introduction
 - [About Zen Cart](0.about_zen_cart.html)

## Read Me First

- Implementation Guide (PDF) - See the /docs folder in your download.
- [Installing Zen Cart](1.readme_installation.html)
- [Upgrading Zen Cart](2.readme_how_to_upgrade.html)
- [Securing your Zen Cart Store](important_site_security_recommendations.html)

## What's New - Features and Bug Fixes

- [What's New in v2.3.x](whatsnew_2.3.0)
- [What's New in v2.2.x](whatsnew_2.2.0)
- [What's New in v2.1.x](whatsnew_2.1.0)
- [What's New in v2.0.x](whatsnew_2.0.0.html)
- [What's New in v1.5.8](whatsnew_1.5.8.html)
- [What's New in v1.5.7](whatsnew_1.5.7.html)

## Changed Files

- [Changed in v2.3.x](changed_files-v2-3-0)
- [Changed in v2.2.x](changed_files-v2-2-0)
- [Changed in v2.1.x](changed_files-v2-1-0)
- [Changed in v2.0.x](changed_files-v2-0-0.html)
- [Changed in v1.5.8](changed_files-v1-5-8.html)
- [Changed in v1.5.7](changed_files-v1-5-7.html)

## Help and Support
- [Documentation](https://docs.zen-cart.com/)
- [Support Forum](https://www.zen-cart.com/forum.php)

## Coming Soon
- [What's Next](whatsnext)

## Archive
- [What's New  docs for older releases](/release/archive_whatsnew/)
- [Changed Files docs for older releases](/release/archive_changed/)


