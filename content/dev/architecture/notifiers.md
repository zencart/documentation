...
One of the many goals of the Zen Cart project has always been to make it simple for third party developers to add functionality to the core code in an easy and unobtrusive manner. To do this we use the [override system](/user/template/template_overrides/), the [auto inclusion system](/dev/architecture/inclusion/) and the observer/notifier system.
...
3. [Loading Your Observer Class](#loading-your-observer-class).  Identifies how to load your observer-class so that it can begin its observations.  You may choose between [automatic loading](/dev/architecture/notifiers/#auto-loaded-observers) and [manual loading](/dev/architecture/notifiers/#manually-loaded-observers). 
...
Event names are strings and are triggered via a `notify` function.  You can see a [list of notifiers](/dev/architecture/notifiers_list/) in the Zen Cart core code for reference. 
...
* A [list of notifiers](/dev/architecture/notifiers_list/) for the current Zen Cart release. 
* The [output of the Zen Cart Notifier Report](/dev/architecture/notifier_report/) run against the current release is provided on the docs site for easy reference by developers.  
